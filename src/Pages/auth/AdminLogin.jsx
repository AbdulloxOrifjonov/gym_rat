...
function AdminLogin() {
  ...
  const [addLogin, { isLoading, isError }] = useAddLoginMutation();
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-md p-8 bg-white shadow-lg rounded-lg">
        ...
        <form className="space-y-6" onSubmit={handleSubmit(onSubmit)}>
          ...
          <Button
            type="submit"
            disabled={isLoading} // Yuklanayotganini ko'rsatish
            className="w-full bg-indigo-600 text-white hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-300 font-semibold py-2 px-4 rounded-lg"
          >
            {isLoading ? "Yuklanmoqda..." : "Sign In"} {/* Yuklanmoqda deb yozish */}
          </Button>
        </form>
      </div>
    </div>
